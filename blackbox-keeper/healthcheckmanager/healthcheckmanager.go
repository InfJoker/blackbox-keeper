...
func NewManager(config configuration.Config) *HealthCheckManager { // TODO parse more parameters from config especially stop-action
...
func (h *HealthCheckManager) RunChecks(processManager *processmanager.ProcessManager) {
	for name, check := range h.checkers {
		err := check.Check()
		if err != nil { // TODO do this smarter
			log.Printf("Restarting app %s...", name)
			err = processManager.KillProcess(name)
			if err != nil {
				log.Printf("Failed stopping the app %s", name)
			}
			err = processManager.StartProcess(name)
			if err != nil {
